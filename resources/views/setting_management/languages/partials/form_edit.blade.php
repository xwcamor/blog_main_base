<form id="form-save" action="{{ route('setting_management.languages.update', $language) }}" method="POST" data-parsley-validate>
  @csrf
  @method('PUT')
  ...
  <div class="form-group">
    ...
    <input type="text" name="locale" id="locale" value="{{ old('locale', $language->locale) }}"
           class="form-control" maxlength="10" placeholder="Ej: en, es_ES">
  </div>

  <div class="form-group">
    <label for="flag">{{ __('languages.flag') }}</label>
    <input type="text" name="flag" id="flag" value="{{ old('flag', $language->flag) }}"
           class="form-control" maxlength="255" placeholder="Ej: us.png">
  </div>
