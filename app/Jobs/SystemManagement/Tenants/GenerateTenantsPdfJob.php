...
class GenerateTenantsPdfJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        try {
            ...
            $tenants = Tenant::filter($this->filters)
                ->with('creator')
                ->get();

            // Generate PDF with options
            $pdf = Pdf::loadView('system_management.tenants.pdf.template', compact('tenants'))
            ...
            \Log::error('GenerateTenantsPdfJob failed', [
                'error' => $e->getMessage(),
    ...
    {
        return __('tenants.export_filename') . '_' . now()->format('Y-m-d_H-i-s') . '.pdf';
    }
}
